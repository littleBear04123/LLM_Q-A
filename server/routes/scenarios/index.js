...
const { createInitialStatusTable, statusTableToText, generateCollectedInfoSummary } = require('../../utils');
...
async function getScenarioStatusTable(scenarioId) {
    try {
        ...
        // 从数据库获取当前场景的状态表
        const statusTable = scenarioModel.getScenarioStatusTable(scenarioId);
...
async function updateScenarioStatusTable(scenarioId, statusTable) {
    try {
        await scenarioModel.updateScenarioStatusTable(scenarioId, statusTable);
    } catch (error) {
...
async function getRecentChatHistory(scenarioId) {
    ...
    try {
        // 从数据库获取指定场景的消息历史
        const recentMessages = messageModel.getMessageHistory(scenarioId, 50); // 获取最近50条消息
        ...
        for (const [component, updateData] of Object.entries(components)) {
            ...
            if (updateData) {
                ...
                if (updateData.summary !== undefined && updateData.summary !== '') {
                    ...
                    // 如果有摘要内容，不管AI返回什么状态，都应该设为collected
                    updatedTable[mappedField][component].status = 'collected';
                    console.log(`自动更新状态为collected: ${mappedField}.${component}`);
                } else {
                    // 如果没有摘要内容，才根据AI的状态更新
...
async function callDeepSeekAPI(prompt, context = null) {
    ...
    try {
        ...
        // 使用导入的专业对话系统提示词
        let systemPrompt = DIALOGUE_SYSTEM_PROMPT + `
...
请根据已有信息和用户的新输入，提出下一个相关问题。`;
        ...
        try {
            ...
            const response = await fetch('https://api.deepseek.com/chat/completions', {
                ...
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${process.env.DEEPSEEK_API_KEY || 'sk-852776dbae374cd0897001aa991243c6'}`
                },
                body: JSON.stringify({
                    model: 'deepseek-chat',
                    messages: [
                        { role: 'system', content: '你是一个有用的AI助手，能够回答各种问题和提供帮助。' },
            ...
            console.error('通用聊天处理错误:', error);
            res.status(500).json({ error: '聊天处理失败: ' + error.message });
        }
    }
});
...
router.post('/generate', async (req, res) => {
    ...
    try {
        ...
        // 从场景相关的消息历史获取上下文
        let scenarioDB = scenarioModel.getScenarioByUseCase(useCaseId);
        if (!scenarioDB) {
            // 如果没有找到场景，创建一个新场景
            scenarioDB = {
                id: scenarioModel.createScenario(useCaseId, title || "需求收集场景", initialInput || "")
            };
        }
        ...
        // 使用简单场景API生成完整场景文档（返回字符串而非JSON对象）
        const scenario = await callSimpleScenarioAPI(generationPrompt, SIMPLE_GENERATION_PROMPT);
        ...
        if (useCaseId) {
            ...
            if (scenarioDB.id) {
                ...
                // 更新现有场景的内容和状态表
                scenarioModel.updateScenarioContent(scenarioDB.id, cleanedScenario, JSON.stringify(currentStatusTable));
            } else {
                // 创建新场景（这种情况理论上不应该发生，因为我们已经在前面创建了）
                scenarioDB.id = scenarioModel.createScenario(useCaseId, title, cleanedScenario);
            }
...
请生成简洁但完整的场景描述：`;
        ...
        if (useCaseId) {
            ...
            if (scenarioDB) {
                ...
                // 创建新场景
                scenarioId = scenarioModel.createScenario(useCaseId, title, cleanedScenario);
            }
            
            console.log('保存场景到数据库，ID:', scenarioId);
            
            // 对于简单生成，状态表为空
            scenarioModel.updateScenarioContent(scenarioId, cleanedScenario, JSON.stringify(createInitialStatusTable()));
        }
...
// 保存编辑后的场景内容
router.post('/save-edited', async (req, res) => {
    if (!req.session || !req.session.user_id) {
        return res.status(401).json({ error: '请先登录' });
    }
    ...
    try {
        ...
        // 获取用户现有的场景（如果有）
        const existingScenarios = scenarioModel.getScenariosByUseCase(useCaseId);
        let scenarioId;
        ...
        if (existingScenarios && existingScenarios.length > 0) {
            ...
            // 更新现有场景的内容和状态表
            scenarioModel.updateScenarioContent(scenarioId, content, JSON.stringify(currentStatusTable));
            console.log('更新现有场景，ID:', scenarioId);
        } else {
            // 如果没有现有场景，则创建新场景
            scenarioId = scenarioModel.createScenario(useCaseId, `Edited Scenario for Use Case ${useCaseId}`, content);
...
// 生成场景图API
router.post('/:useCaseId/generate-diagram', async (req, res) => {
    if (!req.session || !req.session.user_id) {
        return res.status(401).json({ error: '请先登录' });
    }
    ...
    try {
        ...
        // 从数据库获取场景内容
        const scenario = scenarioModel.getScenarioByUseCase(useCaseId);
        ...
        if (scenario) {
            ...
            await scenarioModel.updateScenarioContent(scenario.id, scenario.generated_scenario || scenarioContent, JSON.stringify(statusTable), plantUmlCode);
        } else {
            // 如果场景不存在，创建新场景并保存PlantUML代码
            const scenarioId = scenarioModel.createScenario(useCaseId, `Scenario for Use Case ${useCaseId}`, contentToUse);
            // 获取新创建的场景并更新PlantUML代码
            const newScenario = scenarioModel.getScenarioByUseCase(useCaseId);
            if (newScenario) {
                const statusTable = newScenario.status_table ? JSON.parse(newScenario.status_table) : {};
                await scenarioModel.updateScenarioContent(newScenario.id, contentToUse, JSON.stringify(statusTable), plantUmlCode);
            }
        }
...
router.get('/:useCaseId/messages', async (req, res) => {
    if (!req.session || !req.session.user_id) {
        return res.status(401).json({ error: '请先登录' });
    }
    ...
    try {
        const useCaseId = req.params.useCaseId;
        console.log('=== 获取消息历史API ===');
        console.log('UseCaseId:', useCaseId);
        
        // 获取与用例关联的场景，如果不存在则返回空消息历史
        const scenario = scenarioModel.getScenarioByUseCase(useCaseId);
        ...
        if (scenario) {
            // 如果场景存在，获取该场景的消息历史
            console.log('从场景ID获取消息历史:', scenario.id);
            messages = messageModel.getMessageHistory(scenario.id);
            console.log('获取到的消息数量:', messages.length);
        } else {
            console.log('未找到关联场景，返回空消息历史');
        }
        // 如果场景不存在，返回空消息历史而不报错
...
// 获取场景内容API
router.get('/:useCaseId/get-scenario', async (req, res) => {
    if (!req.session || !req.session.user_id) {
        return res.status(401).json({ error: '请先登录' });
    }
...
// 获取场景图代码API
router.get('/:useCaseId/diagram', async (req, res) => {
    if (!req.session || !req.session.user_id) {
        return res.status(401).json({ error: '请先登录' });
    }
...
9.控制活动图的长度和复杂度，避免生成过于庞大复杂的图表（一般不超过20-30个节点或步骤）
10.如果场景比较复杂，请优先展示核心流程，简化次要分支
...
module.exports.callSimpleScenarioAPI = callSimpleScenarioAPI;
module.exports.getRecentChatHistory = getRecentChatHistory;
module.exports.getScenarioStatusTable = getScenarioStatusTable;
module.exports.createInitialStatusTable = createInitialStatusTable;
module.exports.mergeStatusUpdates = mergeStatusUpdates;
module.exports.updateScenarioStatusTable = updateScenarioStatusTable;
module.exports.countCollectedItems = countCollectedItems;
module.exports.parseAIReply = parseAIReply;